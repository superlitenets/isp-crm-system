...
class HuaweiOLT {
    ...
    public function getONUDetailedInfo(int $oltId, ?int $slotFilter = null): array {
        ...
        $sql = "SELECT id, frame, slot, port, onu_id, sn, name, description, status, snmp_status, rx_power, tx_power, distance, last_down_cause, online_since, updated_at FROM huawei_onus WHERE olt_id = ?";
        $params = [$oltId];
        ...
        return [
            ...
            'onus' => $allOnus,
            'count' => count($allOnus),
            'slots_scanned' => array_keys($slotsFound)
        ];
    }
    ...
    public function authorizeONU(int $onuId, int $profileId, string $authMethod = 'sn', string $loid = '', string $loidPassword = '', array $options = []): array {
        ...
        if ($tr069Vlan && $assignedOnuId !== null) {
            ...
            // Determine TR-069 gemport based on line profile
            // Line profile 2 (SMARTOLT_FLEXIBLE_GPON) has gemport 2 for TR-069
            $tr069GemPort = $options['tr069_gem_port'] ?? $profile['tr069_gem_port'] ?? (($profile['line_profile'] ?? 0) == 2 ? 2 : 1);
    ...
    public function authorizeONUStage1(int $onuId, int $profileId, array $options = []): array {
        ...
        if ($tr069Vlan && $assignedOnuId !== null) {
            ...
            // Determine TR-069 gemport based on line profile
            // Line profile 2 (SMARTOLT_FLEXIBLE_GPON) has gemport 2 for TR-069
            // Line profile 1 (one-isp-lp) only has gemport 1
            $tr069GemPort = $options['tr069_gem_port'] ?? null;
            if ($tr069GemPort === null) {
                // Use gemport 2 for SmartOLT-style profiles, gemport 1 for others
                $tr069GemPort = ($lineProfileId == 2) ? 2 : 1;
            }
    ...
    public function configureONUStage2TR069(int $onuDbId, array $options = []): array {
        ...
        $tr069GemPort = $options['tr069_gem_port'] ?? ($lineProfileId == 2 ? 2 : 1);
        $tr069TrafficIndex = $options['tr069_traffic_index'] ?? 7;
    ...
    public function configureWANViaTR069(int $onuDbId, array $config): array {
        ...
        $sendTask = function($task, $taskName = null, $triggerConnection = false) use ($genieacsUrl, $genieacsId, $onuDbId, $onu, $self) {
            ...
            // Direct POST with connection_request and timeout
            // Now that presets are removed, this should execute immediately
            $url = "{$genieacsUrl}/devices/" . urlencode($genieacsId) . "/tasks";
            if ($triggerConnection) {
                $url .= "?connection_request&timeout=30000";
            }
            $ch = curl_init($url);
            // Longer timeout for connection_request (device needs time to connect + execute)
            ...
            // For connection_request tasks, log the result
            if ($triggerConnection) {
                error_log("[GENIEACS] Task with connection_request result: HTTP {$httpCode}");
            }
        ...
        if ($wanMode === 'pppoe') {
            error_log("[WAN_CONFIG] Starting PURE TR-069 PPPoE config for ONU {$onuDbId}, GenieACS ID: {$genieacsId}");
            
            // PURE TR-069 approach - NO OMCI
            // All WAN configuration is done directly via TR-069 setParameterValues
            $pppoeUsername = $config['pppoe_username'] ?? '';
            $pppoePassword = $config['pppoe_password'] ?? '';
            
            // Step 1: Refresh WAN tree in GenieACS to discover existing paths
            $refreshWanTask = [
                'name' => 'getParameterValues',
            ...
            if ($result['success']) {
                $tasksSent[] = 'PPPoE credentials sent to device';
            } else {
                $errors[] = "PPPoE setParameterValues failed (HTTP {$result['code']})";
            }
    ...
    public function configureWiFiViaTR069(int $onuDbId, array $config): array {
        ...
        $requestJson = json_encode($task);
        $ch = curl_init("{$genieacsUrl}/devices/" . urlencode($genieacsId) . "/tasks?connection_request&timeout=30000");
        curl_setopt_array($ch, [
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_POST => true,
            CURLOPT_POSTFIELDS => $requestJson,
    ...
    public function deleteONUFromOLT(int $onuId, bool $async = true): array {
        ...
        if ($onuIdNum !== null && $onuIdNum !== '') {
            try {
                ...
                $sendRaw = function(string $cmd, int $timeout = 15000) use ($oltId) {
                    $r = $this->callOLTService('/execute-raw', [
                        'oltId' => (string)$oltId,
                        'script' => $cmd,
                        'timeout' => $timeout
                    ]);
                ...
                $spOutput = $sendRaw("display service-port port {$frame}/{$slot}/{$port} ont {$onuIdNum}", 20000);
                error_log("[DeleteONU] Service-port query: " . substr(str_replace(["\r\n", "\n"], " | ", $spOutput), 0, 500));
                
                $servicePortIds = [];
                $spLines = explode("\n", $spOutput);
                foreach ($spLines as $line) {
                    if (preg_match('/^\s*(\d+)\s+\d+\s+\w+\s+gpon/i', $line, $m)) {
                        $servicePortIds[] = (int)$m[1];
                    }
                }
                error_log("[DeleteONU] Found " . count($servicePortIds) . " service-ports: " . implode(', ', $servicePortIds));
                ...
                $output = $deleteOut;
                $oltSuccess = preg_match('/Number of ONTs that have been deleted|success:\s*1/i', $output);
                $hasSPFailure = preg_match('/Failure.*service/i', $output);
                if (!$oltSuccess || $hasSPFailure) {
                    $oltWarning = ' (OLT cleanup may need manual action: ' . trim(preg_replace('/\s+/', ' ', substr($output, -200))) . ')';
                    error_log("[DeleteONU] WARNING: " . substr($output, -300));
                } else {
                    error_log("[DeleteONU] OLT delete successful");
                }
    ...
    public function getTR069ProfileId(): int {
        try {
            ...
            return $profileId ? (int)$profileId : 3;
        } catch (\Exception $e) {
            return 3;
        }
    }
    ...
    public function getTR069Profiles(int $oltId): array {
        $cmd = "display ont tr069-server-profile all";
        $result = $this->executeCommand($oltId, $cmd);
        ...
        foreach ($lines as $line) {
            $line = trim($line);
            if (empty($line) || strpos($line, '---') !== false || stripos($line, 'Profile ID') !== false || stripos($line, 'Total:') !== false) {
                continue;
            }
            
            if (preg_match('/^\s*(\d+)\s+(.+?)\s+(\d+)\s*$/', $line, $m)) {
                $profileId = (int)$m[1];
                $profileName = trim($m[2]);
                $bindingTimes = (int)$m[3];
                
                $details = $this->getTR069Profile($oltId, $profileId);
                
                $profiles[] = [
                    'profile_id' => $profileId,
                    'profile_name' => $profileName,
                    'binding_times' => $bindingTimes,
                    'acs_url' => $details['profile']['acs_url'] ?? '',
                    'conn_req_username' => $details['profile']['conn_req_username'] ?? '(empty)',
                    'conn_req_password' => $details['profile']['conn_req_password'] ?? '(empty)'
                ];
            }
        }
    ...
    public function getTR069Profile(int $oltId, int $profileId): array {
        $cmd = "display ont tr069-server-profile profile-id {$profileId}";
        $result = $this->executeCommand($oltId, $cmd);
    ...
    public function clearTR069ProfileCredentials(int $oltId, int $profileId): array {
        ...
        $cmd = "ont tr069-server-profile modify profile-id {$profileId} connection-request-username \"\" connection-request-password \"\"";
        $result = $this->executeCommand($oltId, $cmd);
    ...
    public function updateTR069Profile(int $oltId, int $profileId, array $settings): array {
        ...
        if (isset($settings['acs_url'])) {
            $commands[] = "ont tr069-server-profile modify profile-id {$profileId} acs-url {$settings['acs_url']}";
        }
        
        if (isset($settings['periodic_inform'])) {
            $enable = $settings['periodic_inform'] ? 'enable' : 'disable';
            $interval = $settings['periodic_interval'] ?? 300;
            $commands[] = "ont tr069-server-profile modify profile-id {$profileId} periodic-inform {$enable} periodic-inform-interval {$interval}";
        }
        
        if (isset($settings['clear_credentials']) && $settings['clear_credentials']) {
            $commands[] = "ont tr069-server-profile modify profile-id {$profileId} connection-request-username \"\" connection-request-password \"\"";
        }
