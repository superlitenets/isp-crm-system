...
class RadiusBilling {
    ...
    public function registerHotspotDeviceByPhone(string $phone, string $mac, int $packageId): array {
        ...
        if ($customer) {
            ...
            $stmt = $this->db->prepare("
                INSERT INTO customers (account_number, name, phone, address, service_plan, created_at) VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
            ");
            $stmt->execute([$accountNumber, 'Hotspot User ' . substr($phone, -4), '+' . $phone, 'Hotspot', 'Hotspot']);
