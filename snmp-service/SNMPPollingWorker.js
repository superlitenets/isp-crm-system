...
class SNMPPollingWorker {
    constructor() {
        ...
        this.oltServiceUrl = process.env.OLT_SERVICE_URL || 'http://localhost:3002';
    ...
    async isOltLocked(oltId) {
        try {
            const resp = await axios.get(`${this.oltServiceUrl}/status/${oltId}`, { timeout: 3000 });
            return resp.data?.exclusiveLock || false;
        } catch {
            return false;
        }
    }

    async isOltPaused(oltId) {
        try {
            const resp = await axios.get(`${this.oltServiceUrl}/discovery/status`, { timeout: 3000 });
            return false;
        } catch {
            return false;
        }
    ...
    async executeCliCommand(oltId, command, timeout = 30000) {
        try {
            const resp = await axios.post(`${this.oltServiceUrl}/execute`, {
                oltId: oltId.toString(),
                command,
                timeout
            }, { timeout: timeout + 5000 });
            return resp.data?.output || '';
        } catch (e) {
            console.log(`[SNMP-CLI] Execute failed for OLT ${oltId}: ${e.message}`);
            return '';
        }
    }

    async executeCliRaw(oltId, script, timeout = 15000) {
        try {
            const resp = await axios.post(`${this.oltServiceUrl}/execute-raw`, {
                oltId: oltId.toString(),
                script,
                timeout
            ...
            console.log(`[SNMP-CLI] Execute-raw failed for OLT ${oltId}: ${e.message}`);
            return '';
        }
    }

    async isOltSessionConnected(oltId) {
        try {
            const resp = await axios.get(`${this.oltServiceUrl}/status/${oltId}`, { timeout: 3000 });
    ...
    async pollOlt(olt) {
        return new Promise(async (resolve, reject) => {
            const locked = await this.isOltLocked(olt.id);
            ...
            try {
                ...
                if (onuStatuses.length === 0 && !locked) {
                ...
                } else if (onuStatuses.length === 0 && locked) {
                    console.log(`[SNMP] OLT ${olt.name} locked for operation - skipping CLI activity`);
                }
                ...
                if (locked) {
                    console.log(`[SNMP] OLT ${olt.name} SNMP failed but locked for operation - skipping CLI fallback`);
                    await this.updatePollingStats(olt.id, false, 'Locked for operation');
                    resolve({ olt: olt.name, onuCount: 0, method: 'skipped-locked' });
                    return;
                }
    ...
    async pollOltViaCli(olt) {
        const locked = await this.isOltLocked(olt.id);
        if (locked) {
            console.log(`[CLI] OLT ${olt.name} locked - skipping CLI poll`);
        ...
        const connected = await this.isOltSessionConnected(olt.id);
        if (!connected) {
            try {
                ...
                if (configResult.rows.length > 0) {
                    ...
                    await axios.post(`${this.oltServiceUrl}/connect`, {
                        oltId: olt.id.toString(),
                        host: cfg.ip_address,
                        port: connectPort,
                        sshPort: connectPort,
        ...
        try {
            for (const [portKey, onus] of Object.entries(groupedByPort)) {
                if (await this.isOltLocked(olt.id)) {
                ...
                try {
                    ...
                    const result = await this.executeCliCommand(olt.id, cmd, 30000);
                    ...
                    if (offlineOnus.length > 0) {
                        ...
                        for (const { onu } of newlyOffline) {
                            try {
                                const script = `interface gpon ${frame}/${slot}\ndisplay ont info ${port} ${onu.onu_id}\nquit\n`;
                                const infoResult = await this.executeCliRaw(olt.id, script, 15000);
                                if (infoResult) {
        ...
        try {
            await this.collectOpticalViaCLI(olt, groupedByPort);
        } catch (e) {
    ...
    async collectOpticalViaCLI(olt, portMap) {
        if (await this.isOltLocked(olt.id)) {
        ...
        for (const portKey of portKeys) {
            ...
            try {
                ...
                const result = await this.executeCliRaw(olt.id, script, 20000);
                if (!result) continue;
                ...
                for (const pk of portKeys) {
                    ...
                    try {
                        ...
                        for (const onu of group.onus) {
                            ...
                            try {
                                const script = `interface gpon ${group.frame}/${group.slot}\ndisplay ont info ${group.port} ${onu.onu_id}\nquit\n`;
                                const result = await this.executeCliRaw(olt.id, script, 15000);
                                if (result) {
