...
<body>
    ...
    <div class="d-flex">
        ...
        <div class="main-content flex-grow-1 p-4">
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header bg-primary text-white">
                            <h5 class="modal-title"><i class="bi bi-gear me-2"></i>Change ONU Mode</h5>
                            <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                        </div>
                        <div class="modal-body">
                            <div class="mb-3">
                                <label class="form-label fw-bold">Select Mode</label>
                                <select id="onuModeSelect" class="form-select" onchange="onModeChange()">
                                    <option value="Bridge" <?= strtolower($ipMode ?? '') === 'bridge' ? 'selected' : '' ?>>Bridge Mode</option>
                                    <option value="Router" <?= strtolower($ipMode ?? '') !== 'bridge' ? 'selected' : '' ?>>Router Mode</option>
                                </select>
                                <small class="text-muted">Bridge mode: ONU acts as a simple bridge. Router mode: ONU handles NAT/routing.</small>
                            </div>
                            <div id="routerWanOptions" style="display:none;">
                                <div class="mb-3">
                                    <label class="form-label fw-bold">WAN Type</label>
                                    <select id="routerWanType" class="form-select" onchange="onWanTypeChange()">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="number" id="routerServiceVlan" class="form-control" placeholder="Service VLAN" value="<?= htmlspecialchars($currentOnu['vlan_id'] ?? '') ?>">
                                </div>
                                <div id="pppoeFields2" style="display:none;">
                                    <div class="mb-3">
                                        <label class="form-label">PPPoE Username</label>
                                        <input type="text" id="pppoeUser" class="form-control" placeholder="PPPoE Username">
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label">PPPoE Password</label>
                                        <input type="password" id="pppoePass" class="form-control" placeholder="PPPoE Password">
                                    </div>
                                </div>
                                <div id="staticFields2" style="display:none;">
                                    <div class="mb-3">
                                        <label class="form-label">Static IP</label>
                                        <input type="text" id="staticIp" class="form-control" placeholder="192.168.1.100">
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label">Gateway</label>
                                        <input type="text" id="staticGw" class="form-control" placeholder="192.168.1.1">
                                    </div>
                                </div>
                            </div>
                            <div id="modeChangeStatus" class="alert d-none"></div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                            <button type="button" class="btn btn-primary" onclick="saveOnuMode()">
            ...
            function saveOnuMode() {
                const mode = document.getElementById('onuModeSelect').value;
                const statusDiv = document.getElementById('modeChangeStatus');
                statusDiv.className = 'alert alert-info';
                statusDiv.textContent = 'Saving mode change...';
                statusDiv.classList.remove('d-none');
                
                const data = {
                    action: 'update_onu_mode',
                    onu_id: <?= $currentOnu['id'] ?? 0 ?>,
                    mode: mode
                };
                
                if (mode === 'Router') {
                    data.wan_type = document.getElementById('routerWanType').value;
                    data.service_vlan = document.getElementById('routerServiceVlan').value;
                    if (data.wan_type === 'pppoe') {
                        data.pppoe_user = document.getElementById('pppoeUser').value;
                        data.pppoe_pass = document.getElementById('pppoePass').value;
                    } else if (data.wan_type === 'static') {
                        data.static_ip = document.getElementById('staticIp').value;
                        data.static_gw = document.getElementById('staticGw').value;
                    }
                }
                
                fetch('?page=huawei-olt&ajax=update_mode', {
                    method: 'POST',
                ...
                .then(result => {
                    if (result.success) {
                        ...
                        statusDiv.textContent = 'Mode updated successfully!';
                        document.getElementById('onuModeDisplay').textContent = mode;
                        document.getElementById('onuModeDisplay').className = 'badge bg-' + (mode === 'Bridge' ? 'secondary' : 'info');
                        setTimeout(() => {
                            bootstrap.Modal.getInstance(document.getElementById('changeModeModal')).hide();
                            statusDiv.classList.add('d-none');
                        }, 1500);
                    } else {
                        statusDiv.className = 'alert alert-danger';
                        statusDiv.textContent = result.error || 'Failed to update mode';
                    }
                })
                .catch(err => {
                    statusDiv.className = 'alert alert-danger';
            ...
            document.getElementById('changeModeModal')?.addEventListener('shown.bs.modal', function() {
                onModeChange();
            });
            </script>
            <script>
